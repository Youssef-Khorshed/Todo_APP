  ...
  Widget build(BuildContext context) {
    return BlocConsumer<NewsCubit,Newsstatus>(
        builder: (context,status){
          return NewsCubit.science_list.length>0 ? ListView.separated
            (itemBuilder: (context,index)=>news_item(context,NewsCubit.science_list[index]),
              separatorBuilder: (context,index)=>line(),
              itemCount: NewsCubit.science_list.length) : Center(child: CircularProgressIndicator()) ;},
